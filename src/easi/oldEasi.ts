export class Easi {
  ...
  call(methodName: any, callback: any, data: any) {
    let _this = this;
    this.setupWebViewJavascriptBridge(function (bridge: any) {
      ...
      bridge.callHandler(methodName, data, function responseCallback(response: any) {
   ...
   * @returns
   */
  lowVersionTips(methodName: any, callback: any) {
    // 老的Bridge列表
   ...
   * @returns
   */
  callMyApp(bridgeType: any, callback: any, data: any = '') {
    if (this.isEasi) {
  ...
  compareVersionEle(currVersion: any, targetVersion: any) {
  ...
  // 调用分享到朋友圈
  wx_share(url: any, title: any, desc: any, mode: any) {
   ...
   * @returns
   */
  addCart(callback: any, { itemId = null, openDetails = true }) {
    if (!this.appVersion) return callback(this.SYS_ERROR);
    if (!itemId) return callback(this.PARAMES_ERROR);
    if (!!itemId && typeof itemId === 'string') itemId = Number(itemId) as any;
   ...
   * @returns
   */
  showCart(callback: any, { show = false }) {
    if (!this.appVersion) return callback(this.SYS_ERROR);
